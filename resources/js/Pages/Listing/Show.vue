    ...
    <div class="flex flex-col-reverse md:grid md:grid-cols-12 gap-4">
        ...
        <div class="md:col-span-5 flex flex-col gap-4">
            ...
            <div v-if="!listing.sold_at">
                <div v-if="user && !offerMade && user.id !== listing.by_user_id">
                    <MakeOffer @offer-updated="offer = $event" :listing-id="listing.id" :price="listing.price" />
                </div>
                <div v-else class="text-gray-400 font-bold">Please Login to make offer</div>

                <OfferMade v-if="user && offerMade" :offer="offerMade" />
            </div>
            <div v-else>
                <span class="text-green-800 bg-green-400 rounded-md p-2 font-bold">SOLD</span>
            </div>
        </div>
    </div>
</template>
